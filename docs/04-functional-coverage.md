...
|---|---|
| **7Blocks marketplace** (block registry, iframe sandbox, postMessage bridge, wizard, block recipes, favorites, fsSync, ai-format endpoint) | `BLOCKS_AUDIT.md`: **zero workspace installations** across audited blocks; the bridge needs new message types per domain; the flagship `ktp-kalkulator` is a 2.1 MB tenant-branded DB blob. The jobs blocks did (calculators, offer generation, document formatting) are covered by agent tools + document templates. If embeddable widgets return as a requirement, they should be a thin renderer over service APIs — not a parallel platform. |
